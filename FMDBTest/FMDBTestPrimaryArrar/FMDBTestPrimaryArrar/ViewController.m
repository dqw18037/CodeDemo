...
- (void)viewDidLoad {
    ...
    self.dbpath = [docPath stringByAppendingPathComponent:@"dqw23.sqlite"];
}
- (IBAction)createTable:(id)sender {
    ...
    if ([fileManager fileExistsAtPath:self.dbpath] == NO) {
        ...
        FMDatabase * db = [FMDatabase databaseWithPath:self.dbpath];
        if ([db open]) {
            NSString * sql = @"CREATE TABLE 'User' ('id' INTEGER PRIMARY KEY AUTOINCREMENT  NOT NULL , 'name' VARCHAR(30), 'password' VARCHAR(30), 'arr' TEXT, 'arrData' BINARY)";
...
//        NSArray *arr = @[@"text1", @"text2",@"text1", @"text2"];
        NSMutableArray *arr = [[NSMutableArray alloc]init];
        for (int i = 0; i <= 500; i ++) {
            NSString *str = [NSString stringWithFormat:@"dqw dfdfdfdfdfadfdfdfdfadfadhttp://stackoverflow.com/questions/11711769/about-lsqlite3-0-sql-library + %d", i * 111];
            [arr addObject:str];
        }
        ...
        NSString * sql = @"insert into user (name, arr, arrData) values(?, ?, ?) ";
        NSString * name = [NSString stringWithFormat:@"dqw%d", idx++];
        BOOL res = [db executeUpdate:sql, name, arrStr, data];
        if (!res) {
            NSLog(@"%@", @"error");
        }else{
            NSLog(@"%@", @"succ");
...
- (IBAction)queryData:(id)sender {
    ...
    if ([db open]) {
        ...
        while ([res next]) {
            ...
            NSData *arrData = [res dataForColumn:@"arrData"];
            NSArray *dataArr = [NSJSONSerialization JSONObjectWithData:arrData options:NSJSONReadingAllowFragments error:nil];
//            NSLog(@"%@, %@, %@", dataArr, [dataArr class], dataArr[0]);
//            NSLog(@"%d, %@, %@, %@", id , name, pass, arrStr);
            NSLog(@"%d, %@, %@", id , name, pass);

            
//            NSData *data = [arrStr JSONData];
//            NSArray *arr = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
            NSArray *arr = [arrStr objectFromJSONString];
//            NSLog(@"%@, %@, %@", arr, [arr class], arr[0]);
