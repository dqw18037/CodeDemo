...
@property (nonatomic, assign) BOOL isPageControlInside;

@end

@implementation SDCycleScrollView

- (instancetype)initWithFrame:(CGRect)frame pageControlHeight:(CGFloat)pageControlHeight pageControlInside:(BOOL)isPageControlInside
{
    if (self = [super initWithFrame:frame]) {
        _isPageControlInside = isPageControlInside;
...
- (instancetype)initWithFrame:(CGRect)frame pageControlHeight:(CGFloat)pageControlHeight
{
    if (self = [super initWithFrame:frame]) {
        _pageControlHeight = pageControlHeight;
        [self initialization];
...
{
    if (self = [super initWithFrame:frame]) {
        _pageControlHeight = kTKPageControlHeight;
        [self initialization];
        [self setupMainView];
    }
    return self;
...
- (void)initialization
{
    ...
    _pageControlDotSize = CGSizeMake(12, 2);
...
- (void)setupMainView
{
    SDCycleScrollLayout *flowLayout = [[SDCycleScrollLayout alloc] init];
    flowLayout.itemSize = CGSizeMake(self.bounds.size.width, self.bounds.size.height - (!_isPageControlInside ? _pageControlHeight : 0));
    flowLayout.minimumLineSpacing = 0;
    flowLayout.minimumInteritemSpacing = 0;
    flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    _flowLayout = flowLayout;
    
    UICollectionView *mainView = [[UICollectionView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.bounds.size.width, self.bounds.size.height - (!_isPageControlInside ? _pageControlHeight : 0)) collectionViewLayout:flowLayout];
...
- (void)setFrame:(CGRect)frame
{
    ...
    _flowLayout.itemSize = CGSizeMake(self.bounds.size.width, self.bounds.size.height - (!_isPageControlInside ? _pageControlHeight : 0));
}
...
- (void)layoutSubviews
{
    ...
    _flowLayout.itemSize = CGSizeMake(self.bounds.size.width, self.bounds.size.height - (!_isPageControlInside ? _pageControlHeight : 0));
    _mainView.frame = CGRectMake(0.0f, 0.0f, self.bounds.size.width, self.bounds.size.height - (!_isPageControlInside ? _pageControlHeight : 0));
    ...
    CGFloat y = self.frame.size.height - (_pageControlHeight + size.height) / 2;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [self setPageControlCurrentPage:indexOnPageControl];
}
...
@implementation SDCycleScrollLayout
...
{
    NSMutableArray* elementAttributes = [NSMutableArray arrayWithArray:[super layoutAttributesForElementsInRect:rect]];
    
    for (UICollectionViewLayoutAttributes* attributes in elementAttributes) {
